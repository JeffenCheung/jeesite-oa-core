...
//		return "modules/act/actTaskForm";
	...
	/**
	 * 取消签收任务，自动下发到单个人的任务如取消签收将无法找回待办，需提醒终止。
	 * 
	 * @param taskId
	 *            任务ID
	...
	public String unclaim(Act act) {
		Task t = actTaskService.getTask(act.getTaskId());
		try {
			List<Map<String, Object>> list = actTaskService.traceProcess(t
					.getProcessInstanceId());
			for (int i = 0; i < list.size(); i++) {
				Map<String, Object> map = list.get(i);
				if ((Boolean)map.get("currentActiviti")) {
					Map<String, Object> vars = (Map<String, Object>) map
							.get("vars");
					if (StringUtil.isEmpty((String)vars.get("candidateGroups"))) {
						return "自动下发到单个处理人的任务，无法取消签收。请继续任务办理。";
					}
				}
			}
		} catch (Exception e) {
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = "trace/info/light/{proInsId}")
	public List<Map<String, Object>> traceInfoLight(@PathVariable("proInsId") String proInsId) throws Exception {
		List<Map<String, Object>> activityInfos = actTaskService.traceProcessLight(proInsId);
